...
public class EspecialidadeService {
	...
	public Map<String, Object> buscarEspecialidades(HttpServletRequest request) {
		...
		Page<?> page = datatables.getSearch().isEmpty() ? repository
				.findAll(datatables.getPageable()) : repository.findAllByTitulo(datatables.getSearch(), datatables.getPageable());
		return datatables.getResponse(page);
	}

	@Transactional(readOnly = true)
	public Especialidade buscarPorId(Long id) {
		Especialidade esp = repository.findById(id).get();
		return esp;
	...
	@Transactional(readOnly = true)
	public Map<String, Object> buscarEspecialidadeByMedico(Long id, HttpServletRequest request) {
		datatables.setRequest(request);
		datatables.setColunas(DatatablesColunas.ESPECIALIDADES);
